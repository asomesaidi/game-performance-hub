...
import { X, Cpu, Monitor, MemoryStick, HardDrive, Gauge, Palette, Gamepad2, Ruler } from 'lucide-react';
...
export function FilterPanel({
  ...
  const handleClear = () => {
    setCpu(''); setGpu(''); setRam(''); setStorage('');
    onClearSpecs();
    onClose();
  };
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">Enter your specs to see performance results</p>
                </div>
                <button
                  onClick={onClose}
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <input className={inputClass} placeholder="e.g. Intel Core i7-12700K" value={cpu} onChange={e => setCpu(e.target.value)} />
                </div>

                <div>
                  <label className={labelClass}><Monitor className="w-3.5 h-3.5" /> GPU Model *</label>
                  <input className={inputClass} placeholder="e.g. NVIDIA RTX 3070" value={gpu} onChange={e => setGpu(e.target.value)} />
                </div>

                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className={labelClass}><MemoryStick className="w-3.5 h-3.5" /> RAM (GB) *</label>
                    <input className={inputClass} type="number" placeholder="16" value={ram} onChange={e => setRam(e.target.value)} />
                  </div>
                  <div>
